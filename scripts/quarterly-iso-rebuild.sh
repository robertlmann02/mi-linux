...
# Intended runner: an always-on automation host/user timer.
# Intended builder: an x86_64 Linux host reachable over SSH.
set -euo pipefail

export PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin

RUNNER_REPO="${MI_LINUX_RUNNER_REPO:-${MI_LINUX_PI_REPO:-$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/.." && pwd)}}"
BUILDER_HOST="${MI_LINUX_BUILDER_HOST:-builder.example.internal}"
BUILDER_USER="${MI_LINUX_BUILDER_USER:-builder}"
BUILDER_REPO="${MI_LINUX_BUILDER_REPO:-/srv/mi-linux-build/mi-linux}"
REMOTE_URL="${MI_LINUX_REMOTE_URL:-https://github.com/robertlmann02/mi-linux.git}"
BRANCH="${MI_LINUX_BRANCH:-main}"
LOCAL_CANDIDATE_ROOT="${MI_LINUX_LOCAL_CANDIDATE_ROOT:-$RUNNER_REPO/out/quarterly-candidates}"
LOG_ROOT="${MI_LINUX_LOG_ROOT:-$RUNNER_REPO/out/quarterly-rebuild-logs}"
PUBLISH_CANDIDATE_TO_PUBLIC="${MI_LINUX_PUBLISH_CANDIDATE_TO_PUBLIC:-0}"
PUBLIC_CANDIDATE_ROOT="${MI_LINUX_PUBLIC_CANDIDATE_ROOT:-/srv/mi-linux-downloads/quarterly-candidates}"
...
log "runner_repo_head=$LOCAL_HEAD origin_$BRANCH=$ORIGIN_HEAD"
...
if [ "$PUBLISH_CANDIDATE_TO_PUBLIC" = "1" ] && sudo -n true 2>/dev/null; then
  log "copying verified candidate artifacts into public-staging candidate directory"
  sudo mkdir -p "$PUBLIC_CANDIDATE_ROOT/$RELEASE_DATE"
  sudo rsync -a --delete "$LOCAL_OUT/" "$PUBLIC_CANDIDATE_ROOT/$RELEASE_DATE/"
  sudo find "$PUBLIC_CANDIDATE_ROOT/$RELEASE_DATE" -type f -exec chmod 0644 {} +
  sudo find "$PUBLIC_CANDIDATE_ROOT/$RELEASE_DATE" -type d -exec chmod 0755 {} +
  log "public_candidate_out=$PUBLIC_CANDIDATE_ROOT/$RELEASE_DATE"
else
  log "public candidate copy skipped; either disabled or passwordless sudo unavailable"
fi
